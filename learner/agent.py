...
import learner.flappy_freeze as db
...
class Parameters:
    def __init__(self,
                 ...
                 observation_fpa=4,             # frames per action generating trainigngdata
                 training_fpa=1,                # FPA while training
                 ...
                 exploration_steps=500000,      # epsilon decay steps
                 training_epochs=3,
        ...
        self.k = observation_fpa
        self.k_observe = observation_fpa
        self.k_train = training_fpa
        ...
        self.game = flappy.Environment(self.target,
                                       self.mode,
                                       self.FPS)
        ...
        self.replay = db.ReplayCollection(session_id=self.sess_id,
                                          episodes_to_keep=self.buffer,
...
class Agent:
    def __init__(self,
        ...
        self.params = Parameters(state_size, action_size, hidden_size,
                                 ...
                                 training_sample_size, observation_fpa,
                                 training_fpa, max_games, game_difficulty,
                                 fps, keep_gif_for_score, max_game_minutes,
        ...
        self.s_log = '{}session_{}.csv'.\
            format(log_path, self.params.sess_id)

        config = ConfigProto()
        config.gpu_options.allow_growth = True
        self.sess = Session(config=config, graph=tf.get_default_graph())

        # THIS MAY NOT GO HERE - NEEDS THOUROUGH TESTING
        K.set_session(self.sess)
        print('{} | Keras Session Initialized (ID: {})...'.
              format(timestamp(), self.params.sess_id))

        self.model = DeepQ(state_size,
                           action_size,
                           hidden_size,
                           learn_rate,
                           loss_function,
        ...
        try:
            self.model.nn.load_weights('saved\\FlappyBird_model.h5')
            print('{} | {} Saved Keras Model Weights Successfully Loaded...'.
                  format(timestamp(), self.params.alg))
        except:
            print('{} | {} No Saved Keras Model Weights Found...'.
                  format(timestamp(), self.params.alg))

        print('{} | {} Keras Model Successfully Compiled...'.
              format(timestamp(), self.params.alg))
    ...
    def play(self):
        ...
        for ep in range(0, p.episodes):
            ...
            while p.status != 'exit' and p.t < p.steps:
                ...
                p.s_t1 = np.append(p.s_t[0, :, :, 1:], p.x_t1, axis=2)
                ...
                * only store new states past pipe1_x
                ...
                if ep < 30 or p.t >= 50:
                    ...
                    #if p.training:
                    #    print('{} | Ep: {}  State: {} Stored in Replay Memory...'.\
                    #    format(timestamp(), ep + 1, p.t))
                ...
                if p.G > p.observe or p.training:    # train when done observing
                    if not p.training:               # only trigger status change once
                        ...
                        p.k = p.k_train              # set frames per action to training rate
    ...
    def save_model(self, p):
        ...
        self.model.nn.save(p.saves + '/' + p.name + '_model.h5',
                           overwrite=True)
        print('{} | {} Model Saved to {}_model.h5...'.
              format(timestamp(), p.alg, p.name))
        plot_model(self.model.nn, to_file=p.saves +
                   '/' + p.name + '_model.png')
        print('{} | {} Model Plot Saved to {}_model.png...'.
              format(timestamp(), p.alg, p.name))

        '''---Save model weights to separate .h5 file---'''
        self.model.nn.save_weights(p.saves +
    ...
    def action(self, state, p):
        ...
        with self.sess.as_default():
            p.Qs = self.model.nn.predict(state)
        p.maxQ = np.argmax(p.Qs)                 # exploit action: idx of maxQ
    ...
    def replay(self, p):
        ...
        bat = p.memory.sample(p.batch)   # add current state to training batch
        bat.append((p.s_t, p.a_t, p.r_t, p.s_t1, p.terminal))
        ...
        with self.sess.as_default():
            tarQ_bat = self.model.nn.predict(s_bat)       # targetQ values
            Q_bat = self.model.nn.predict(s1_bat)         # future Q values

        '''---Logic to Update maxQ Values'''
        for i, x in enumerate(term_bat):
            '''---tarQ_bat[i] = 0 if Terminal---'''
            tarQ_bat[i, [np.argmax(a_bat[i])]] = (-1) if x else \
        ...
        with self.sess.as_default():
            hist = self.model.nn.train_on_batch(s_bat, tarQ_bat)
            p.hist = hist

    def plot_hist(self, p):
        # Plot training & validation accuracy values
        plt.plot(p.hist.history['acc'])
        plt.plot(p.hist.history['val_acc'])
        ...
        plt.plot(p.hist.history['loss'])
        plt.plot(p.hist.history['val_loss'])
    ...
    def log_session(self, p, elapsed):
        ...
        session_log = {
            'sess_id': p.sess_id, 'k_observe': p.k_observe,
            'time': elapsed, 'state_size': p.S, 'learn_rate': p.lr,
            'k_train': p.k_train, 'memory_size': p.buffer,
            'batch_size': p.batch, 'episodes': p.episodes,
            'max_ep_steps': p.steps, 'score_target': p.target,
            'gif_score': p.gif_score, 'observe': p.observe,
            'anneal': p.anneal, 'action_size': p.A, 'filter_size': p.H
        }
